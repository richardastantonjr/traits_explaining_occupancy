...
for (i in c(10)){         
  ...
  ## calculate WAIC for each species
  occ.lppd <-  -2*sum(colMeans(out.rep$sims.list$log_pd)) #posterior predictive density
  occ.pd <- sum((apply(out.rep$sims.list$log_pd, 2, sd))^2) #penalty based on SD of log_pd
  occ.WAIC[i] <- occ.lppd + 2 * occ.pd
} 
...
## Spp. 10 [280,000], 18, 27, 29, 38, 42 [250,000]

## Save results
#saveRDS(occ.WAIC, "./outputs/WAIC_interaction_models_2")
