...
##----------------------
## plot species precicted occupancy across range of shrub cover 
## for species supported by WAIC
tiff("./outputs/Fig_S_spp_int.tiff", width = 18, height = 18, units = 'in', res = 300)
par(mfrow = c(4, 5))
for (j in int_supported) {
...
load("./outputs/Occupancy_analysis_Swazi_birds_Bayesian_quadTime_simplified_20180505.RData")
...
## For how many species is predicted occupancy higher with the 
...
## int_supported comes from compare_WAIC.R
source("./code/code/compare_WAIC.R")
sum(pred_home_int[int_supported] > pred_home[int_supported]) 
sum(pred_pasture_int[int_supported] > pred_pasture[int_supported])

##-------------
## models with no interaction and sugar cane included
load("./code/code/Occupancy_analysis_Swazi_birds_Bayesian_quadTime_fixed.RData")
Outputs <- list(Output1,Output2,Output3,Output4,Output5,
                Output6,Output7,Output8,
                Output9,Output10,Output11,Output12,
                Output13,Output14,Output15,Output16,
...
tiff("./outputs/Figures/Fig4.tiff", width = 6.5, height = 6, units = 'in', res = 300)
plot(o.shrub[1:119], pmC_home[1:119], col = "turquoise1", lwd = 3, type = 'l', lty = 1, frame = FALSE, ylim = c(0, 1),
     ylab = "Mean occupancy", las = 1, xlim = c(0,100), cex.lab = 1.6,
     xlab = "Shrub cover, %")
 ##matlines(o.shrub[1:119], criC_home[1,1:119], col = "turquoise1", lty = 1)
 ##matlines(o.shrub[1:119], criC_home[2,1:119], col = "turquoise1", lty = 1)
...
tiff("./outputs/Fig_S_spp.tiff", width = 18, height = 18, units = 'in', res = 300)
##par(mfrow = c(6, 8))
par(mfrow = c(4, 5))
#for (j in 1:48) {  ## unlock to print predictions for all species
  for (j in int_supported) {
  plot(o.shrub[1:119], pred_home[j,1:119], col = "turquoise1", lwd = 3, type = 'l', lty = 1, frame = FALSE, 
       ylim = c(0, 1), ylab = "Mean occupancy", las = 1, xlim = c(0, 100),
       xlab = "Shrub cover, %", main = dimnames(AbunHists.subset)[[3]][j])
