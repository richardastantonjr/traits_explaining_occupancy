...
load("./data/Occupancy_analysis_Swazi_birds_Bayesian_quadTime_fixed.RData")
...
## Subtract any species not observed in a land use from the total
...
main_efx_rows <- data.frame (matrix (c("Community-level effect", res_pooled_95[3,1],
                  res_pooled_95[3,2], res_pooled_95[3,3], "Pasture", 
                  885, ##max(as.numeric(sppEfx_minus_int$RobertsNum)) + 1,
                  "Inconclusive",
                  
                  "Community-level effect", res_pooled_95[4,1],
                  res_pooled_95[4,2], res_pooled_95[4,3], "Homestead", 
                  885, "Neg",
                  
                  "Community-level effect", res_pooled_95[5,1],
                  res_pooled_95[5,2], res_pooled_95[5,3], "Plantation", 
                  885, "Neg",
                  
                  "Community-level effect", res_pooled_95[2,1],
                  res_pooled_95[2,2], res_pooled_95[2,3], "Shrub cover", 
                  885, "Pos"
                  ), nrow = 4, byrow = TRUE) )
colnames(main_efx_rows) <-  colnames(sppEfx_minus_int)
sppEfx_minus_int <- rbind.data.frame(sppEfx_minus_int, main_efx_rows)
sppEfx_minus_int$effect <- as.numeric(sppEfx_minus_int$effect)
sppEfx_minus_int$LCL <- as.numeric(sppEfx_minus_int$LCL)
sppEfx_minus_int$UCL <- as.numeric(sppEfx_minus_int$UCL)
...
forest_plot_spp <- ggplot(data = sppEfx_minus_int, aes(x = Species, y = effect)) + 
  ...
  scale_x_discrete(limits = rev(sppEfx_minus_int[order(sppEfx_minus_int$RobertsNum),]$Species), expand = c(0.02, 0.01)) +
  coord_flip() +  
...
## This section (to line 690) produces figures for a talk that do not appear in the manuscript
...
nest.fig <- ggplot(data = nest.summary_minus_int, aes(x = betaName, y = effect, group = nest)) +
  ...
  geom_point(colour = "gray20", shape = 21, size = 5, position = pd, aes(fill = factor(nest))) + 
  ...
  coord_flip() +
  theme_bw() +
  theme(plot.title = element_text(size = 33, face = "bold"),
        axis.title.x = element_text(face = "bold", vjust = 0.3, size = 30,
                                    colour = "black"),
        axis.text.x  = element_text(vjust = 0.5, size = 30, colour = "black",
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        legend.text = element_text(size = 30), legend.title = element_text(face = "bold",
            size = 30))
...
diet.fig <- ggplot(data = diet.summary_minus_int, aes(x = betaName, y = effect, group = diet)) + 
  geom_errorbar(aes(ymin = LCL, ymax = UCL), width = 0, size = 0.3, position = pd) +
  geom_point(colour = "gray20", shape = 21, size = 5, position = pd, aes(fill = factor(diet))) + 
  ...
  theme(plot.title = element_text(size = 33, face = "bold"),
        ...
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        legend.text = element_text(size = 30), legend.title = element_text(face = "bold", size = 30))
...
fig3 <- plot_grid(nest.fig, diet.fig, labels = c("a)", "b)"), 
                  label_size = 33, ncol = 2)
...
ggplot2::ggsave(file = "./outputs/Figures/figure2.tiff", fig2, width = 12, height = 10, units ="in", dpi = 600)
ggplot2::ggsave(file = "./outputs/Figures/figure3.tiff", fig3, width = 18, height = 14, units ="in", dpi = 300)
